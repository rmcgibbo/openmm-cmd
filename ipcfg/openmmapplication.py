...
class OpenMMApplication(Application):
    ...
    def initialize(self):
        ...
        if len(config) > 0:
            ...
            self.load_config_file(config_file_path, not any(a == 'make_config' for a in sys.argv))
    ...
    def print_options(self):
        ...
        lines.append('    Path to a configuration file to load from (or to save to, if using `make_config`).')
        print os.linesep.join(lines)
    ...
    def __init__(self, config={}):
        for key in config[self.__class__.__name__].keys():
            if key not in self.class_traits():
                self.application.error('%s has no configurable trait %s' % (self.__class__.__name__, key))
    ...
    def config_section(self):
        ...
        for name, trait in self.__class__.class_traits(config=True).iteritems():
            ...
            if name in self.active:
                lines.append('c.%s.%s = %r' % (self.__class__.__name__, name, getattr(self, name)))
            else:
                lines.append('# c.%s.%s = %r' % (self.__class__.__name__, name, trait.get_default_value()))
            lines.append('')
        return '\n'.join(lines)
